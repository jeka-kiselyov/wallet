{include file="admin/breadcrumbs.tpl" title="New template"}
...
<form method="post">
  ...
  <div class="form-group">
  <label  for="subject">{t}Template identificator{/t}</label>   <input type="text" name="name" value="{if isset($smarty.post.name)}{$smarty.post.name|escape:'html'}{/if}"  id="name" class="form-control" placeholder="{t}Template identificator{/t}">
  </div>

  <div class="form-group">
  <label  for="subject">{t}Subject{/t}</label>   <input type="text" name="subject" value="{if isset($smarty.post.subject)}{$smarty.post.subject|escape:'html'}{/if}"  id="subject" class="form-control" placeholder="{t}Subject{/t}" >
  </div>

  <div class="form-group">
  <label  for="subject">{t}Body{/t}</label>   <textarea id="body" name="body">{if isset($smarty.post.body)}{$smarty.post.body|escape:'html'}{/if}</textarea>
  </div>
